...
Bamboo.prototype.buildPlan = function(params, planKey, callback) {
    params = params || {};
    var planUri = [this.host, 'rest','api','latest', 'queue', planKey + '.json'].join('/') + '?' + _.map(params, function(v, k) { return [encodeURIComponent(k),encodeURIComponent(v)].join('=')}).join('&');
    request({uri: planUri, method:'POST'}, function(error, response, body) {
        if (error || response.statusCode !== 200) {
            callback("Can not build plan");
            return false;
        }
